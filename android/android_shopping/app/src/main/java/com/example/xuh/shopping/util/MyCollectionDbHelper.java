...
public class MyCollectionDbHelper extends SQLiteOpenHelper {
    ...
    public void addMyCollection(Collection collection) {
        ...
        values.put("nicheng",collection.getNicheng());
     ...
     * @param price 价格
     */
    public void deleteMyCollection(String title,String description,String nicheng) {
        SQLiteDatabase db = this.getWritableDatabase();
        if(db.isOpen()) {
            db.delete(DB_NAME,"title=? and description=? ",new String[]{title,description});
            db.close();
        }
    }
